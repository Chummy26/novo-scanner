...
description: Use esta skill sempre que o assunto for projetar, raciocinar sobre, auditar ou justificar o dataset histórico que treina o modelo da estratégia coberta pela skill mãe `spread-arbitrage-strategy` — i.e., o histórico que substitui, para o modelo, a leitura mental que o operador faz quando abre a rota e olha o passado dela antes de decidir se entra. Cobre, em nível conceitual: (i) onde este dataset se encaixa entre famílias de datasets de ML financeiro e o que ele explicitamente não é, (ii) a separação entre observação contínua e amostra de supervisão e os três papéis funcionais que qualquer materialização precisa preencher, (iii) as identidades não-negociáveis (point-in-time strict com ordem de operações em t₀, `entry_locked` imutável vs. `exit` estocástico, label nunca como melhor saída ex-post, multi-horizonte estrutural com monotonicidade, censura à direita como primeira ordem, falsificabilidade), (iv) como Testes 1 e 2 da skill mãe se materializam em features e label, (v) a fronteira estrutural vs. discricionário, (vi) a separação entre o que é necessário para um primeiro modelo defensável e o que é complementar, (vii) os riscos do dataset e como cada um condiciona o comportamento real-time do modelo. Esta skill **depende** da skill mãe; quando houver conflito, a skill mãe vence. Não substitui spec de schema implementado — é o documento conceitual a partir do qual o spec é derivado.
---
...
**`EntryContextT0` derivado.** Se UI, auditoria ou trainer quiserem uma leitura chamada informalmente de `entry_quality`, ela deve ser derivada desses componentes point-in-time e permanecer decomponível. O dataset não deve substituir ranks, distâncias, escala robusta e cobertura por um escalar opaco, nem misturar `p_exit_ge_label_floor_minus_entry_*` nessa leitura. Exequibilidade da saída pertence ao Teste 2; qualidade relativa da entrada pertence ao Teste 1.
...
- **`ExitTargetPolicy` do modelo.** A política que escolhe um `exit_target` único entre vários candidatos estimados pertence à camada de decisão operacional do modelo, não ao label. Coleta e primeiro treino podem operar em modo estimador (`P_hit`, `T`, `P_censor`, quantis e `IC` por floor/horizonte) sem escolher um setup primário. O dataset deve suportar uma futura escolha única com multi-floor, multi-horizonte, censura, `t_realize` e `S_saída(t_realize)`, mas não deve embutir uma utility específica como verdade supervisionada.
...
- **Identificadores e tempo:** ID estável de rota, `sample_id` determinístico e timestamp em nanosegundos (§3.1). O registro também precisa carregar os componentes auditáveis da rota — `canonical_symbol`, `buy_venue`, `sell_venue`, `buy_market`, `sell_market` — para permitir validações diretas de ausência de SPOT/SPOT, `sell_market = FUTURES_PERP` e `buy_venue != sell_venue` sem depender de parsing opaco do `route_id`. Para amostras rotuladas, materializar também `label_window_closed_at_ns = ts_emit_ns + horizon_s * 1e9`; isso torna `cutoff_train ≤ T_now − max(H)` testável sem recomputação externa.
- **Estado em `t₀`:** `entry_locked` (§3.2), `exit_start = S_saída(t₀)` e a decomposição do baseline mecânico de book (`half_spread_buy_now`, `half_spread_sell_now`). Sem `exit_start`, a foto de `t₀` não fecha a identidade instantânea `entry + exit`, o Teste 2 não é auditável contra o estado observado, e a recomendação `{enter, exit, lucro_bruto, P, T, IC}` perde contexto de comparação. Sem os half-spreads, a distância ao baseline estrutural `entry + exit = -2*(half_spread_buy + half_spread_sell)` fica inferida demais para auditoria de parser/book.
- **Features point-in-time da rota:** quantis rolantes de `entry` e `exit` em **ao menos uma janela curta e uma janela longa** (Testes 1 e 2 da skill mãe; §4.1, §4.2). O mínimo operacional recomendado é manter 24 h como referência, adicionar uma janela curta (ex.: `1h`) e uma longa (ex.: `7d`) com estatísticas suficientes para cauda de entrada e probabilidade histórica de saída contra `label_floor - entry_locked`.
...
- **Tempo vivo da oportunidade em `t₀`** quando houver threshold operacional: `time_alive_at_t0_s` mede há quanto tempo a rota está continuamente acima do critério de oportunidade antes da emissão. É feature PIT, não outcome, e permite auditar peer-effect/crowding sem olhar o futuro.
