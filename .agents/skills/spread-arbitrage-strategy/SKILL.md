...
description: Use esta skill sempre que o assunto for explicar, raciocinar sobre, ou ensinar a estratégia manual de arbitragem de spread cross-venue operada sobre cripto — i.e., a estratégia em que o operador abre duas pernas simultâneas (compra numa venue, venda noutra) travando um spread de entrada favorável, e depois espera o spread de saída melhorar para fechar com lucro. Cobre, em nível conceitual: (i) a taxonomia técnica desse tipo de arbitragem, (ii) o significado preciso de "spread de entrada" e "spread de saída" como duas séries distintas derivadas dos mesmos quatro preços, (iii) a identidade de PnL bruto `PnL = S_entrada(t₀) + S_saída(t₁)`, (iv) a heurística humana de decisão de entrada (comparar o spread de entrada atual com a distribuição das últimas ~24h E verificar que o spread de saída historicamente passa por valores favoráveis), (v) a fronteira entre o que é estrutural (vem do mercado, é fixo) e o que é decisão pessoal do operador (threshold de entrada, tempo de hold, meta de lucro — variam por pessoa, não são regras). É uma aula sobre a estratégia em si — não cobre execução de ordens, automação, ML, nem detalhes específicos de implementação.
---
