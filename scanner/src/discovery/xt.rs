...
struct SpotResp    { result: SpotResult }
#[derive(Debug, Deserialize)]
struct SpotResult  { symbols: Vec<SpotSym> }
#[derive(Debug, Deserialize)]
...
#[derive(Debug, Deserialize)]
struct FutResp  { result: Vec<FutSym> }
#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct FutSym {
    symbol: String,
        ...
        match self.venue {
            Venue::XtSpot => {
                ...
                for s in r.result.symbols {
                    if !s.state.is_empty() && s.state.to_uppercase() != "ONLINE" { continue; }
            ...
            Venue::XtFut => {
                let r: FutResp = http.get(self.url())
                    .send().await?.error_for_status()?.json().await?;
                for s in r.result {
                    if s.state != 1 && s.state != 0 { continue; }
                    if let Some(c) = normalize::parse(self.venue, &s.symbol) {
                        out.push(VenueSymbol { venue: self.venue, raw: s.symbol, canonical: c });
                    }
                }
