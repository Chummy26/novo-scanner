...
impl Adapter for BitgetAdapter {
    ...
    async fn run(&self, store: Arc<BookStore>, shutdown: AdapterShutdown) -> Result<()> {
        run_reconnecting("bitget", BackoffPolicy::BITGET, shutdown, || {
            self.run_once(&store)
        })
        .await
    }
...
impl BitgetAdapter {
    ...
    fn build_subscribe_batch(&self, batch_size: usize) -> Vec<String> {
        ...
        for (raw, _id) in per_venue_map {
            args.push(format!(
                r#"{{"instType":"SPOT","channel":"ticker","instId":"{}"}}"#,
                raw
            ));
    ...
    F: FnOnce(&str, Price, Price),
{
    ...
    let bid = Price::from_f64(
        bid_s
            .parse::<f64>()
            .map_err(|_| Error::Decode("bid f64".into()))?,
    );
    let ask = Price::from_f64(
        ask_s
            .parse::<f64>()
            .map_err(|_| Error::Decode("ask f64".into()))?,
    );
    f(sym, bid, ask);
    Ok(())
}
...
mod tests {
    ...
    fn parse_sample_frame() {
        ...
        parse_and_apply(js, |sym, bid, ask| {
            got = Some((sym.into(), bid, ask));
        })
        .unwrap();
